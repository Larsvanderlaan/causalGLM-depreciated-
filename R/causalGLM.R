...
causalGLMwithLASSO <- function(formula, W, A, Y, estimand = c("CATE", "OR", "RR"), cross_fit = TRUE,weights = NULL,data_list = NULL,...  )  {
  ...
  causalGLM(formula, W, A, Y, estimand,sl3_Learner_Y = lrnr, learning_method = "autoHAL", cross_fit = cross_fit, weights = weights, num_knots_Y0W = 1, max_degree_Y0W =1, data_list = data_list  )
}
