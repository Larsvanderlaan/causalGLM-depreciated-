...
# This will take a few seconds. Default is learning_method = autoHAL which is a regularized sparse/smoothness adaptive regression spline algorithm. To make it faster change: learning_method or max_degree_Y0W or num_knots_Y0W or parallel.
# 
causal_fit <- causalGLM(formula = formula_CATE, W  = W, A = A, Y = Y,  max_degree_Y0W = 1, estimand = "CATE")
# We got pretty close!
summary(causal_fit)
...
# causalGLM in high dimensional settings using the LASSO

```{r}
library(sl3)
set.seed(1500)
# Lets make it high dimensional
data_list <- sim.CATE(n=250, p=100)
# Confounders
W <- data_list$W
# Binary treatment
A <- data_list$A
...
causal_fit <- causalGLMwithLASSO(formula = formula_CATE, cross_fit = TRUE, W  = W, A = A, Y = Y,  estimand = "RR")
summary(causal_fit)
# Wow, we got close!
```
