...
n <- 50
p <- 5
data_sim <- sim.CATE(n=n, p=p, formula_estimand =  ~1 + W1 + W2 + W3 + W4, formula_A = ~ ., formula_Y0W =~.)
...
```
## CATE
The estimating equation for CATE is fairly simple (it is linear). Thus, estimating equation estimators should perform fairly well for the CATE relative to non-linear estimating equation estimators (like for RR).
### n = 50, p=5
...
#p=4 is number of covariates in W
### This will print updates of coverage per iteration to your consol. It should run in a minute or so.
 out <-  sim.causalGLM(cross_fit = F, formula = ~1 + W1 + W2 + W3 + W4, n=n, p = p, learning_method = "glmnet", nsims = nsims, estimand= "CATE", compare_with_competitor = T, print_CI_length = T)
...
### This will print updates of coverage per iteration to your consol. It should run in a minute or so.
 ...
 out <-  sim.causalGLM(cross_fit = F, formula = ~1 + W1 + W2  , n=n, p = p, learning_method = "glmnet", nsims = nsims, estimand= "RR", compare_with_competitor = T)
...
### You should see causalGLM do quite a bit better than the estimating equation estimator. Even so, both methods under cover here.
...
#p=4 is number of covariates in W
### This will print updates of coverage per iteration to your consol. It should run in a minute or so.
  out <-  sim.causalGLM(cross_fit = F, formula = ~1 + W1 +W2 + W3 + W4   , n=n, p = p, learning_method = "mars", nsims = nsims, estimand= "OR", compare_with_competitor = T)
...
#p=4 is number of covariates in W
### This will print updates of coverage per iteration to your consol. It should run in a minute or so.
 out <-  sim.causalGLM(cross_fit = F, formula = ~1 + W1 + W2 + W3 + W4, n=n, p = p, learning_method = "gam", nsims = nsims, estimand= "OR", compare_with_competitor = T, include_glmnet_in_stack = F)
...
# Single coefficient comparison

We now compare the methods when only an intercept is modelled for the estimand. Both methods perform very similarly (and very well).
...
#p=4 is number of covariates in W
### This will print updates of coverage per iteration to your consol. It should run in a minute or so.
 out <-  sim.causalGLM(cross_fit = F, formula = ~1 , n=n, p = p, learning_method = "mars", nsims = nsims, estimand= "CATE", compare_with_competitor = T )
# This seed seems to be pretty noisy so it takes awhile to get 95% coverage for both.
...
#p=4 is number of covariates in W
### This will print updates of coverage per iteration to your consol. It should run in a minute or so.
 out <-  sim.causalGLM(cross_fit = F, formula = ~1    , n=n, p = p, learning_method = "glmnet", nsims = 100, estimand= "RR", compare_with_competitor = T , print_CI_length = F)
...
#p=4 is number of covariates in W
### This will print updates of coverage per iteration to your consol. It should run in a minute or so.
 out <-  sim.causalGLM(cross_fit = F, formula = ~1 , n=n, p = p, learning_method = "glmnet", nsims = nsims, estimand= "OR", compare_with_competitor = T )
