...
fit <- glm.fit(data[,c("A", "W1", "W2")], data$Y, family = gaussian() )
...
betaA <- rnorm(p)
betaA <- 2.5 * betaA / sum(betaA)
betaY <- rnorm(p)
betaY <- 2.5 * betaY / sum(betaY)
g1 <- plogis( W %*% betaA)
...
out <- spRR(formula_logRR = ~ 1, W, A, Y, family_RR = gaussian(), sl3_Lrnr_A = Lrnr_glm$new(), sl3_Lrnr_Y = Lrnr_glm$new(), smoothness_order = 1, max_degree = 2, num_knots = c(1,1), fit_control = list())
print(out)
...
D <- D +
  ...
  node("W4", distr = "runif", min = -1, max = 1) +
  node("A", distr = "rbinom", size = 1, prob = plogis( (W1 + W2   ) )) +
  node("mu", distr = "rconst", const =   (1 + W1 + W2 + W3 + W4)*A*0.5 + (W1 + W2  + W3 + W4  )) +
  node("Y", distr = "rnorm", mean = mu, sd = 2) 
...
out <- spCATE(formula_CATE = ~ 1 + W1 + W2 + W3 + W4, W, A, Y, family_CATE = gaussian(), sl3_Lrnr_A = Lrnr_earth$new(), sl3_Lrnr_Y  = Lrnr_earth$new(), constant_variance = T,smoothness_order_Y0W = 1, max_degree_Y0W = 1, num_knots_Y0W = c(1,1 ), fit_control = list( parallel  = T)  )
out1 <- out$coefs1
out <- out$coefs
 cate <- 0.5
passes <- cbind(passes, out[,4] <= cate  & out[,5] >= cate )
print(rowMeans(passes))

out <- out1
 
passes1 <- cbind(passes1, out[,4] <= cate  & out[,5] >= cate )
